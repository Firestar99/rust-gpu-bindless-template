...
use my_app_shader::triangle::{Param, Vertex};
use rust_gpu_bindless::descriptor::{
    Bindless, BindlessAllocationScheme, BindlessBufferCreateInfo, BindlessBufferUsage,
    BindlessImageUsage, BindlessInstance, DescriptorCounts, Format, Image2d, MutDesc, MutImage,
    RCDescExt,
};
use rust_gpu_bindless::pipeline::DrawIndirectCommand;
use rust_gpu_bindless::pipeline::{
    BindlessGraphicsPipeline, ClearValue, ColorAttachment, GraphicsPipelineCreateInfo, LoadOp,
    MutImageAccessExt, PipelineDepthStencilStateCreateInfo, PipelineInputAssemblyStateCreateInfo,
    PipelineRasterizationStateCreateInfo, Present, RenderPassFormat, RenderingAttachment, StoreOp,
};
...
use rust_gpu_bindless_winit::ash::{
    ash_enumerate_required_extensions, AshSwapchain, AshSwapchainParams,
    SwapchainImageFormatPreference,
};
...
pub async fn main_loop(
    ...
    let (window, window_extensions) = event_loop
        .spawn(|e| {
            let window =
                e.create_window(WindowAttributes::default().with_title("swapchain triangle"))?;
...
impl TriangleRenderer {
    pub fn new(bindless: &Bindless, rt_format: Format) -> anyhow::Result<Self> {
        ...
        let pipeline = bindless.create_graphics_pipeline(
            ...
            my_app::shader::triangle::triangle_vertex::new(),
            my_app::shader::triangle::triangle_fragment::new(),
        )?;
    ...
    pub async fn draw(
        &mut self,
        rt: MutDesc<MutImage<Image2d>>,
    ) -> anyhow::Result<MutDesc<MutImage<Image2d>>> {
